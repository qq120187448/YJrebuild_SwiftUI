    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                ...
                Text("unlock pro features").foregroundColor(.secondary)
                VStack(spacing: 8) { ForEach(["watermark_free","high_res","unlimited_ocr","batch_export","lidar_boost","no_ads"], id:\.self) { f in HStack { Text(f).foregroundColor(.white); Spacer(); Text("free").foregroundColor(.secondary); Spacer(); Text("pro").foregroundColor(Color(hex: "E74C3C")) }.font(.system(size: 13)).padding(.vertical, 8) } }.padding(18).background(Color.white.opacity(0.06)).clipShape(RoundedRectangle(cornerRadius: 16)).padding(.horizontal, 20)
                ...
                HStack { Text("monthly").foregroundColor(.secondary); Spacer(); Text("25/month").foregroundColor(.secondary).fontWeight(.bold) }.padding(18).background(Color.white.opacity(0.06)).clipShape(RoundedRectangle(cornerRadius: 16)).padding(.horizontal, 20)
