...
enum RoomDataProcessor {
    ...
    /// Applies user-edited component dimensions and room length/width overrides
    /// to a captured room. Returns the original room when no adjustments exist.
    static func applyingAdjustments(
        to room: CapturedRoom,
        adjustments: RoomAdjustments
    ) -> CapturedRoom {
        guard !adjustments.components.isEmpty || adjustments.roomDimensions != nil else {
            return room
        }

        var adjusted = room
        adjusted.walls = adjustedSurfaces(
            adjusted.walls,
            adjustments: adjustments.components
        )
        adjusted.doors = adjustedSurfaces(
            adjusted.doors,
            adjustments: adjustments.components
        )
        adjusted.windows = adjustedSurfaces(
            adjusted.windows,
            adjustments: adjustments.components
        )
        adjusted.openings = adjustedSurfaces(
            adjusted.openings,
            adjustments: adjustments.components
        )
        adjusted.objects = adjustedObjects(
            adjusted.objects,
            adjustments: adjustments.components
        )

        if let dimensions = adjustments.roomDimensions,
           let length = dimensions.length,
           let width = dimensions.width,
           length > 0.05,
           width > 0.05 {
            adjusted.floors = adjustedFloors(
                adjusted.floors,
                length: length,
                width: width
            )
    ...
    private static func adjustedSurfaces(
        _ surfaces: [CapturedRoom.Surface],
        adjustments: [String: ComponentAdjustment]
    ) -> [CapturedRoom.Surface] {
        surfaces.map { surface in
            guard let adjustment = adjustments[surface.identifier.uuidString] else {
                return surface
            }
            var updated = surface
            var dx = 0.0
            if let width = adjustment.width, width > 0.01 {
                dx = (Double(updated.dimensions.x) - width) / 2
                updated.dimensions.x = Float(width)
            }
            if let height = adjustment.height, height > 0.01 {
                updated.dimensions.y = Float(height)
            }
            if let depth = adjustment.depth, depth > 0.01 {
                updated.dimensions.z = Float(depth)
            }
            if abs(dx) > 0.0001 {
                let column = updated.transform.columns.3
                updated.transform.columns.3 = SIMD4<Float>(
                    column.x - Float(dx),
                    column.y,
                    column.z,
                    column.w
    ...
    private static func adjustedObjects(
        _ objects: [CapturedRoom.Object],
        adjustments: [String: ComponentAdjustment]
    ) -> [CapturedRoom.Object] {
        objects.map { object in
            guard let adjustment = adjustments[object.identifier.uuidString] else {
                return object
            }
            var updated = object
            var dx = 0.0
            ...
            if let depth = adjustment.depth, depth > 0.01 {
                dz = (Double(updated.dimensions.y) - depth) / 2
                updated.dimensions.y = Float(depth)
            }
            if let height = adjustment.height, height > 0.01 {
                updated.dimensions.z = Float(height)
            }
    ...
    private static func adjustedFloors(
        ...
        floors.map { floor in
            var updated = floor
            let corners = floor.polygonCorners
            guard corners.count >= 3 else { return floor }
            let xs = corners.map { Double($0.x) }
            let ys = corners.map { Double($0.y) }
            guard let minX = xs.min(), let maxX = xs.max(),
                  let minY = ys.min(), let maxY = ys.max() else { return floor }
            let currentW = maxX - minX
            let currentD = maxY - minY
            guard currentW > 0.01, currentD > 0.01 else { return floor }
            let scaleX = length / currentW
            let scaleZ = width / currentD
            let centerX = (minX + maxX) / 2
            let centerY = (minY + maxY) / 2
            updated.polygonCorners = corners.map { corner in
                simd_float3(
                    Float(centerX + (Double(corner.x) - centerX) * scaleX),
                    Float(centerY + (Double(corner.y) - centerY) * scaleZ),
                    corner.z
                )
            }
            return updated
